...
func Run(args []string, stdin io.Reader, stdout, stderr io.Writer) int {
	...
	format := flags.String("format", "text", "출력 형식: text, svg 또는 html")
	version := flags.Bool("version", false, "버전 출력")
	if err := flags.Parse(args); err != nil {
		if errors.Is(err, flag.ErrHelp) {
			fmt.Fprintln(stderr, "사용법: term-diagram [-ascii] [-fit] [-format text|svg|html] [-width cells] [-height cells] [-f path|-] [-version]")
		} else {
	...
	if *format != "text" && *format != "svg" && *format != "html" {
		fmt.Fprintln(stderr, "출력 형식은 text, svg 또는 html이어야 함")
		return 2
	}
	if *version {
